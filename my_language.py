...
class Lexer:
	...
	def make_tokens(self):
		...
		while self.current_char is not None:
			if self.current_char == '\t':  # Handle tabs
				self.advance()
			elif self.current_char == '\n':  # Handle new lines
				self.advance()
			elif self.current_char == ' ':  # Handle spaces
				self.advance()                
			elif self.current_char == '#':  # Handle comments
				self.advance()  # Move past the initial #
				while self.current_char != '\n' and self.current_char != '#':
					if self.current_char is None:  # End of file check
						break
					self.advance()
				if self.current_char == '#':  # Move past the closing #
					self.advance()
			...
			elif self.current_char in LETTERS:  # Handle identifiers
...
def run(fn, text, line_number=1):
    ...
    ast = parser.parse()
    if ast.error: 
        ast.error.line_number = line_number  # Attach line number to the error
        return [0], ast.error
